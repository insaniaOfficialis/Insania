...
/// <summary>
/// Запросы работы с цветами глаз
/// </summary>
/// <param name="configuration">Интерфейс конфигурации</param>
public class EyesColorsRequests(IConfiguration configuration) : IEyesColors
{
    /// <summary>
    ...
    public async Task<BaseResponseList> GetEyesColorsList()
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(_configuration["Api:EyesColors"])) throw new InnerException(Errors.EmptyUrlEyesColors);

            //Формируем ссылку запроса
            string url = _configuration["Api:Url"] + _configuration["Api:Version"] + _configuration["Api:EyesColors"] + "list";
