...
/// <summary>
/// Запросы работы с типами лиц
/// </summary>
/// <param name="configuration">Интерфейс конфигурации</param>
public class TypesFacesRequests(IConfiguration configuration) : ITypesFaces
{
    /// <summary>
    ...
    public async Task<BaseResponseList> GetTypesFacesList()
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(_configuration["Api:TypesFaces"])) throw new InnerException(Errors.EmptyUrlTypesFaces);

            //Формируем ссылку запроса
            string url = _configuration["Api:Url"] + _configuration["Api:Version"] + _configuration["Api:TypesFaces"] + "list";
