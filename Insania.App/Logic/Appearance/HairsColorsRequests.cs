...
/// <summary>
/// Запросы работы с цветами волос
/// </summary>
/// <param name="configuration">Интерфейс конфигурации</param>
public class HairsColorsRequests(IConfiguration configuration) : IHairsColors
{
    /// <summary>
    ...
    public async Task<BaseResponseList> GetHairsColorsList()
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(_configuration["Api:HairsColors"])) throw new InnerException(Errors.EmptyUrlHairsColors);

            //Формируем ссылку запроса
            string url = _configuration["Api:Url"] + _configuration["Api:Version"] + _configuration["Api:HairsColors"] + "list";
