...
namespace Insania.App.Logic.Appearance;

/// <summary>
/// Запросы работы с типами телосложений
/// </summary>
/// <param name="configuration">Интерфейс конфигурации</param>
public class TypesBodiesRequests(IConfiguration configuration) : ITypesBodies
{
    /// <summary>
    ...
    public async Task<BaseResponseList> GetTypesBodiesList()
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(_configuration["Api:TypesBodies"])) throw new InnerException(Errors.EmptyUrlTypesBodies);

            //Формируем ссылку запроса
            string url = _configuration["Api:Url"] + _configuration["Api:Version"] + _configuration["Api:TypesBodies"] + "list";
