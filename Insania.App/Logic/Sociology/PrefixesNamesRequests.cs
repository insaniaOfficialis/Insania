...
namespace Insania.App.Logic.Sociology;

/// <summary>
/// Запросы работы с префиксами имён
/// </summary>
/// <param name="configuration">Интерфейс конфигурации</param>
public class PrefixesNamesRequests(IConfiguration configuration) : IPrefixesNames
{
    /// <summary>
    ...
    public async Task<BaseResponseList> GetList(long? nationId)
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(_configuration["Api:PrefixesNames"])) throw new InnerException(Errors.EmptyUrlPrefixesNames);

            //Формируем ссылку запроса
            NameValueCollection queryParam = System.Web.HttpUtility.ParseQueryString(string.Empty);
            if (nationId != null) queryParam.Add("nationId", nationId.ToString());
            string? param = queryParam?.ToString();
            string url = _configuration["Api:Url"] + _configuration["Api:Version"] + _configuration["Api:PrefixesNames"] + "list" +
